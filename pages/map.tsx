...
// this way it's possible to create custom map marker
const marker = {
...
const Map = ({ focusedCoords }: { focusedCoords?: Coords }) => {
  ...
  const mappedPoints = (): any[] =>
    useMemo(() => {
      return waypointsFull.map((waypoint, index) => {
        if (index < waypointsFull.length - 1 && response && response.routes[0].legs[index]) {
          return {
            ...waypoint,
  ...
  return (
    <>
      <LoadScript googleMapsApiKey={apiKey} libraries={libs as any[]}>
        <div>
